...
public class UnusedPrivateFieldProcessor extends AbstractProcessor<CtField<?>> {

  @Override
  public boolean isToBeProcessed(CtField<?> candidate) {
    if (candidate.getAnnotation(SuppressWarnings.class) != null) {
    ...
    boolean legitAccess = rootPackage.getElements(new SameFieldAccessFilter(ctFieldReference)).stream()
      .filter(ref -> {
        return ref.getParent(CtReturn.class) != null || ref.getParent(CtIf.class) != null
          || ref.getParent(CtInvocation.class) != null || ref.getParent(CtExpression.class) != null || ref.getParent(CtLoop.class) != null;
      }).findAny().isPresent();

    if (!legitAccess) {
      System.out.println("Need to remove :" + ctField.getReference().getQualifiedName());
      // System.out.println(ctField.getParent().getClass());
